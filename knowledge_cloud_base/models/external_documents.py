...
class ExternalDocuments(models.Model):
    ...
    def _compute_object_date(self):
        for record in self:
            if record.object_id:
                ...
                elif record.object_id._name == 'crm.lead':
                    record.object_date = record.object_id.date_assign
                else:
    ...
    def restore_attachment_from_cloud(self):
        ...
        # _logger.info("INVOKE RESTORE %s:%s" % (self.ir_attachment_id, self))
    ...
    def un_share_attachment_from_cloud(self):
        _logger.info("INVOKE UNSHARE %s:%s" % (self.ir_attachment_id, self))
