...
class MrpTechnicalDocuments(models.Model):
    _inherit = "mrp.technical.documents"
    ...
    def _links_get(self):
        ...
        return [(r.object, r.name) for r in link_obj.search([('object', 'in', ['sale.order', 'purchase.order',
                                                                               ...
                                                                               'crm.lead', 'mrp.routing.workcenter'])])]
    ...
    def create(self, vals):
        if 'attachment_journal_id' not in vals:
            if self._context.get('attachment_journal'):
                ...
                attachment_journal_id = self.env['ir.attachment.journal'].search([('journal_type', '=', 'mrp')],
                                                                                 limit=1)
        ...
        return super(MrpTechnicalDocuments, self).create(vals)
    ...
    def write(self, vals):
        ...
        return super(MrpTechnicalDocuments, self).write(vals)
